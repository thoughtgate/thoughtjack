...
/// a document link within that group. When `id_map` is provided,
/// scenario paths are resolved to their metadata IDs (used as the
/// MDX filename); otherwise the filename stem is used.
///
/// Implements: TJ-SPEC-011 F-004
#[must_use]
pub fn generate_sidebars_js<S: BuildHasher>(
    registry: &Registry,
    ...
    for category in &registry.categories {
        ...
        for scenario_path in &category.scenarios {
            let doc_id = id_map
                .get(scenario_path)
                .cloned()
                .unwrap_or_else(|| scenario_path_to_doc_id(scenario_path));
...
mod tests {
    use super::*;
    use crate::docgen::registry::{Category, Registry, SiteConfig};
    use std::collections::HashMap;
    ...
    fn test_generate_sidebars() {
        ...
        let js = generate_sidebars_js(&registry, &HashMap::new());
    ...
    #[test]
    fn test_generate_sidebars_with_id_map() {
        let registry = test_registry();
        ...
        id_map.insert(
            PathBuf::from("scenarios/phased/rug-pull.yaml"),
            "tj-atk-002".to_string(),
        );
        let js = generate_sidebars_js(&registry, &id_map);

        assert!(js.contains("'scenarios/tj-atk-001'"));
        assert!(js.contains("'scenarios/tj-atk-002'"));
        // resource.yaml has no mapping, falls back to filename
        assert!(js.contains("'scenarios/injection/resource'"));
    }
