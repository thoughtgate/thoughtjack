...
use crate::config::schema::{
    ...
    SideEffectConfig, SideEffectType, TimeoutBehavior, Trigger,
};
...
fn prose_for_side_effect(effect: &SideEffectConfig) -> String {
    ...
    match effect.type_ {
        SideEffectType::NotificationFlood => rate.map_or_else(
            || "Triggers notification flood side effect.".to_string(),
            |r| format!("Triggers notification flood side effect at {r} notifications/sec."),
        ),
        SideEffectType::BatchAmplify => "Triggers batch amplification side effect.".to_string(),
        SideEffectType::PipeDeadlock => "Triggers pipe deadlock side effect.".to_string(),
        SideEffectType::CloseConnection => "Closes the connection.".to_string(),
        SideEffectType::DuplicateRequestIds => "Sends duplicate request IDs.".to_string(),
    }
}
...
mod tests {
    use super::*;
    use crate::config::schema::SendNotificationConfig;
