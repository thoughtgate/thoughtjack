...
pub fn validate_cmd(args: &DocsValidateArgs) -> Result<(), ThoughtJackError> {
    ...
    let mut scenarios_for_dup: Vec<(String, crate::config::schema::ScenarioMetadata)> = Vec::new();
...
fn load_registry(path: &Path) -> Result<crate::docgen::registry::Registry, ThoughtJackError> {
