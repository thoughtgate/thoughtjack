...
sidebar_position: 2
description: "Complete YAML configuration schema for ThoughtJack attack scenarios — server identity, tools, resources, prompts, phases, behaviors, and templates."
---

# Configuration Schema

Complete reference for ThoughtJack's YAML scenario configuration format. This documents the per-actor configuration used by built-in scenarios.

:::note Multi-actor documents
Multi-actor OATF documents wrap this per-actor config in an `oatf:` / `attack:` / `execution:` / `actors:` structure. Each actor's `config:` block uses the schema described here.
:::
