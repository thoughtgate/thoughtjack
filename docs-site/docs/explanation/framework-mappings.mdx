...
| Credential phishing via descriptions | T1556 (Modify Authentication Process) | Social engineering through tool interfaces |
...
- **Response sequences** — gradual trust manipulation over multiple calls
...
| MCP-04 | Prompt Injection via Tools | `prompt-injection`, `credential-phishing` |
...
| MCP-09 | Trust Boundary Violation | `rug-pull`, `response-sequence` |
| MCP-10 | Credential Theft | `credential-phishing` |
