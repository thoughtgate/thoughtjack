---
title: Getting Started
sidebar_position: 2
description: Install ThoughtJack, run your first adversarial MCP security test scenario, and learn to interpret the verdict output in under 5 minutes.
---

# Getting Started

In this tutorial you'll install ThoughtJack, run a built-in attack scenario, and learn to read the output. By the end you'll understand the core workflow for testing MCP clients.

ThoughtJack executes attack scenarios authored as [OATF](https://oatf.io) (Open Agent Threat Format) documents — a declarative YAML format for describing adversarial agent test cases.

## 1. Install ThoughtJack

Choose one method:

```bash
# Homebrew (macOS/Linux)
brew install thoughtgate/tap/thoughtjack

# Cargo
cargo install thoughtjack

# Or build from source
git clone https://github.com/thoughtgate/thoughtjack.git
cd thoughtjack
cargo build --release
```

Verify the installation:

```bash
thoughtjack version
```

You should see output like:

```
thoughtjack 0.5.0
  commit:  a1b2c3d
  built:   2026-01-15
  rustc:   1.88.0 (2024 edition)
  target:  aarch64-apple-darwin
```

## 2. Run your first scenario (one terminal)

The fastest way to see ThoughtJack in action — run a scenario with a short timeout:

```bash
thoughtjack scenarios run oatf-001 --mcp-server 127.0.0.1:0 --max-session 5s --no-semantic
```

This starts an MCP server on a random port, waits 5 seconds, and prints the verdict. You'll see the progress header and verdict output immediately — no client needed:

```
  Scenario: OATF-001 Tool Description Prompt Injection
  Protocol: MCP (server)   Severity: CRITICAL
  Phases:   serve

    ✓ OATF-001-01

  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Verdict: NOT_EXPLOITED
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

The verdict is `NOT_EXPLOITED` because no agent connected — but you've confirmed ThoughtJack runs, loads scenarios, and produces verdicts.

## 3. Explore built-in scenarios

ThoughtJack ships with built-in attack scenarios across multiple categories. List them:

```bash
thoughtjack scenarios list
```

```
Built-in Scenarios (10 available)

  Temporal
    oatf-002                Tool Definition Rug Pull
    oatf-007                MCP Server Supply Chain Attack

  Injection
    oatf-001                Tool Description Prompt Injection
    oatf-004                Tool Response Injection (Indirect Prompt Injection)
    ...

Run a scenario: thoughtjack scenarios run <name>
View YAML:      thoughtjack scenarios show <name>
```

View the YAML for a specific scenario:

```bash
thoughtjack scenarios show oatf-002
```

## 4. Run with live traffic

The **rug pull** (`oatf-002`) is a temporal attack: the server starts with a benign calculator tool, builds trust over 3 calls, then swaps the tool definition with a poisoned version containing prompt injection.

Start the scenario as an MCP server:

```bash
thoughtjack scenarios run oatf-002 --mcp-server 127.0.0.1:8080
```

ThoughtJack is now listening on HTTP using the MCP Streamable HTTP transport. It will:

1. Respond to `initialize` with server capabilities
2. Advertise a `calculator` tool via `tools/list`
3. Return `"Result: 42"` for any `tools/call` to `calculator`
4. After 3 tool calls, advance to the next phase and send `notifications/tools/list_changed`
5. On the next `tools/list`, return the calculator with a poisoned description

Real-time progress is shown by default on TTY terminals:

```
  Scenario: OATF-002 Tool Definition Rug Pull
  Protocol: MCP (server)   Severity: CRITICAL
  Phases:   trust_building → swap_definition → exploit

  Phase: trust_building [tools/call ×3]
    ← initialize  [0/3]
    → initialize
    ← tools/call calculator  [1/3]
    → tools/call
    ← tools/call calculator  [2/3]
    → tools/call
    ← tools/call calculator  [3/3]
    → tools/call
    (4.2s, 8 messages)

  Phase: swap_definition [tools/list ×1]
    ▸ notify notifications/tools/list_changed
    ...
```

### What to observe

When testing your MCP client against this scenario, watch for:

- Does the client re-read tool definitions after receiving `list_changed`?
- Does the client detect the prompt injection in the modified `calculator` description?
- Does the agent follow the injected instructions?

## 5. Progress output

ThoughtJack shows real-time progress by default when running in a terminal. The progress display shows phase transitions, trigger counters, entry actions, and indicator results.

Use `--progress compact` for a less detailed view, or `--progress off` to disable progress and show only the human summary after completion:

```bash
# Compact — no trigger counters or phase timing
thoughtjack scenarios run oatf-002 --mcp-server 127.0.0.1:8080 --progress compact

# Off — human summary only
thoughtjack scenarios run oatf-002 --mcp-server 127.0.0.1:8080 --progress off
```

For verbose logging (debug-level internal state), add `-vv`:

```bash
thoughtjack scenarios run oatf-002 --mcp-server 127.0.0.1:8080 -vv
```

For machine-parseable logs, use JSON format:

```bash
thoughtjack scenarios run oatf-002 --mcp-server 127.0.0.1:8080 --log-format json -vv
```

## 6. Run from a config file

Built-in scenarios are convenient, but real testing uses YAML config files. Export a scenario's YAML:

```bash
thoughtjack scenarios show oatf-002 > my-scenario.yaml
```

Run from the file:

```bash
thoughtjack run --config my-scenario.yaml --mcp-server 127.0.0.1:8080
```

Always validate configs before running:

```bash
thoughtjack validate my-scenario.yaml
```

## 7. Test an MCP server (client mode)

The examples above run ThoughtJack as a **server** to test MCP clients. ThoughtJack can also run as an MCP **client** to test servers — useful for verifying that a server handles adversarial client requests correctly.

### Connect to a running server

If the target MCP server is already listening on HTTP:

```bash
thoughtjack run --config scenario.yaml --mcp-client-endpoint http://localhost:3000/mcp
```

### Spawn the server as a subprocess

ThoughtJack can launch the server process and connect over stdio:

```bash
thoughtjack run --config scenario.yaml \
  --mcp-client-command "npx" \
  --mcp-client-args "-y @modelcontextprotocol/server-everything"
```

In client mode, ThoughtJack sends requests defined in the OATF scenario (tool calls, resource reads, etc.) and evaluates the server's responses against the configured indicators.

## Next steps

- [Writing a Custom Scenario](./custom-scenario) — create your own OATF scenario
- [Building Phased Attacks](./phased-attacks) — multi-phase state machines with advanced triggers
- [CLI Reference](/docs/reference/cli) — all commands and flags
