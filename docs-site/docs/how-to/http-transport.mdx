...
```bash
thoughtjack run --config my_config.yaml --http 8080
```
...
thoughtjack run --config my_config.yaml --http 0.0.0.0:8080

# Bind to specific interface
thoughtjack run --config my_config.yaml --http 192.168.1.100:3000
```
...
thoughtjack run --config my_config.yaml --http 8080 --state-scope per-connection

# All connections share phase state
thoughtjack run --config my_config.yaml --http 8080 --state-scope global
```
