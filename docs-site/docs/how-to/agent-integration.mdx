...
For scenarios with multiple MCP servers (e.g., exfiltration chains with a malicious server and a target server), pass multiple `--mcp-server` flags to both ThoughtJack and the agent:
...
# ThoughtJack binds both
thoughtjack run --config scenario.yaml \
  --mcp-server 127.0.0.1:8080 \
  --mcp-server 127.0.0.1:8081 \
  --agui-client-endpoint http://127.0.0.1:8000/
```
