...
| `src/orchestration/runner.rs` (`run_actor`) | Add context-mode path: create per-actor `ServerHandle`s with watch channels and shared `result_tx`/`server_request_tx`; server actors routed through `run_actor()` as in traffic-mode |
| `src/orchestration/runner.rs` (context setup) | AG-UI actor: construct `ContextAgUiDriver` and spawn its `PhaseLoop` directly (not via `run_actor()`) |
